...
public class Piece extends JLabel
{
	...
	public static void move1(MouseEvent me)
	{
		try
		{
			if (CheckerBoard.getClickCount() % 2 == 0)
			{
				...
				for (int i = 0; i < 8; i++)
				{
					for (int j = 0; j < 8; j++)
					{
						if ((initialPoint.getX() == (Checkers.getPoints()[0][i] + Checkers.getGUI().getX() - 5)) && (initialPoint.getY() == (Checkers.getPoints()[1][j] + Checkers.getGUI().getY() - 5)))
						{
			...
			else
			{
				...
				for (int i = 0; i < 8; i++)
				{
					for (int j = 0; j < 8; j++)
					{
						if ((finalPoint.getX() == (Checkers.getPoints()[0][i] + Checkers.getGUI().getX() - 5)) && ((finalPoint.getY() == (Checkers.getPoints()[1][j] + Checkers.getGUI().getY() - 5))))
						{
							...
							if ((!whiteP) || (whiteM == 0))
							{
								CheckerBoard.setToolBar(1, "Player 2 Wins!");
								continue;
							}
							else if ((!blackP) || (blackM == 0))
							{
								CheckerBoard.setToolBar(1, "Player 1 Wins!");
								continue;
							}
							//CPU's turn\\
							...
							for (int a = 0; a < 64; a++)
								if ((getPiece(a).isKing()) && (getPiece(a).getColor().equals(Color.black)))
									moves.add(a);
							for (int b = 0; b < moves.size(); b++)
								if (getCaptureLocations(moves.get(b)).size() == 0)
								{
									moves.remove(b);
									b--;
								}
							if (moves.size() > 0) //If there is a king that can capture.
								cpu = moves.get((int)(Math.random() * moves.size()));
							else
							{
								for (int c = 0; c < 64; c++)
									if (getPiece(c).getColor().equals(Color.black))
										moves.add(c);
								for (int d = 0; d < moves.size(); d++)
									if (getCaptureLocations(moves.get(d)).size() == 0)
									{
										moves.remove(d);
										d--;
									}
								if (moves.size() > 0) //If there is a piece that can capture
									cpu = moves.get((int)(Math.random() * moves.size()));
								else
								{
							...
							int point = getEmptyMoveLocations(cpu).get((int)(Math.random() * getEmptyMoveLocations(cpu).size()));
							JPanel initial = (JPanel)CheckerBoard.getSpaces().get(cpu);
							JPanel end = (JPanel)CheckerBoard.getSpaces().get(point);
							CheckerBoard.getSpaces().set(point, initial);
							CheckerBoard.getSpaces().set(cpu, end);
							System.out.println(cpu);
							if ((point > 55) && (((Piece)initial.getComponent(Piece.CENTER)).getColor().equals(Color.white)))
								getPiece(point).makeKing();
							else if ((point < 8) && (((Piece)initial.getComponent(Piece.CENTER)).getColor().equals(Color.black)))
								getPiece(point).makeKing();
							for (int z = 0; z < 63; z++)
								if (containsPiece(z))
								{
	...
	public static void move2(MouseEvent me)
	{
		try
		{
			if (CheckerBoard.getClickCount() % 2 == 0)
			...
			else
			{
				...
				if ((!whiteP) || (whiteM == 0))
					CheckerBoard.setToolBar(2, "Player 2 Wins!");
				else if ((!blackP) || (blackM == 0))
					CheckerBoard.setToolBar(2, "Player 1 Wins!");
			}
		}
		catch (IndexOutOfBoundsException e){}
		CheckerBoard.setClickCount(CheckerBoard.getClickCount() + 1);
